########################################################################
#
# The transfer-learning Model v3 for TensorFlow.
#
...
# loaded and input to the transfer-learning model, which will then output
...
# images from another data-set than it was trained on.
# For example, the transfer-learning model was trained on the ImageNet
# data-set using a very powerful and expensive computer.
# But the transfer-learning model can be re-used on data-sets it was not
# trained on without having to re-train the entire model,
# even though the number of classes are different for the two data-sets.
# This allows you to use the transfer-learning model on your own data-sets
# without the need for a very powerful and expensive computer to train it.
#
# The last layer of the transfer-learning model before the softmax-classifier
...
# Only output error info about TensorFlow
os.environ["TF_CPP_MIN_LOG_LEVEL"] = '3'

# Directory to store the downloaded data.
data_dir = "./inception/InceptionModel/"
...
# File containing the TensorFlow graphs definition. (Downloaded)
...
class Inception:
    """
    The transfer-learning model is a Deep Neural Network which has already been
    trained for classifying images into 1000 different categories.
    When you create a new instance of this class, the transfer-learning model
    will be loaded and can be used immediately without training.
    The transfer-learning model can also be used for Transfer Learning.
    """
    ...
    # This is used for classifying images with the transfer-learning model.
    ...
    # Name of the tensor for the output of the transfer-learning model.
    ...
    def __init__(self):
        ...
        # Now load the transfer-learning model from file. The way TensorFlow
        # does this is confusing and requires several steps.

        # Create a new TensorFlow computational graphs.
        self.graph = tf.Graph()

        # Set the new graphs as the default.
        with self.graph.as_default():
            ...
            # Open the graphs-def file for binary reading.
            path = os.path.join(data_dir, path_graph_def)
            with tf.gfile.FastGFile(path, 'rb') as file:
                # The graphs-def is a saved copy of a TensorFlow graphs.
                # First we need to create an empty graphs-def.
                graph_def = tf.GraphDef()

                # Then we load the proto-buf file into the graphs-def.
                graph_def.ParseFromString(file.read())

                # Finally we import the graphs-def to the default TensorFlow graphs.
                tf.import_graph_def(graph_def, name='')

                # Now self.graphs holds the transfer-learning model from the proto-buf file.

        # Get the output of the transfer-learning model by looking up the tensor
        ...
        # Get the unscaled outputs for the transfer-learning model (aka. softmax-logits).
        ...
        # Get the tensor for the last layer of the graphs, aka. the transfer-layer.
        ...
        # Create a TensorFlow session for executing the graphs.
        self.session = tf.Session(graph=self.graph)

    def close(self):
        """
        Call this function when you are done using the transfer-learning model.
    ...
    def _write_summary(self, logdir='summary/'):
        """
        Write graphs to summary-file so it can be shown in TensorBoard.
    ...
    def _create_feed_dict(self, image_path=None, image=None):
        ...
        :return:
            Dict for feeding to the transfer-learning graphs in TensorFlow.
        """
    ...
    def classify(self, image_path=None, image=None):
        """
        Use the transfer-learning model to classify a single image.
        ...
        :return:
            ...
            the transfer-learning model thinks the image is of each given class.
        """
        # Create a feed-dict for the TensorFlow graphs with the input image.
    ...
    def get_resized_image(self, image_path=None, image=None):
        """
        Input an image to the transfer-learning model and return
    ...
    def print_scores(self, pred, k=5, only_first_name=False):
        """
    ...
    def transfer_values(self, image_path=None, image=None):
        ...
        These are the values of the last layer of the transfer-learning model before
        the softmax-layer, when inputting the image to the transfer-learning model.
        The transfer-values allow us to use the transfer-learning model in so-called
        ...
        # Use TensorFlow to run the graphs for the transfer-learning model.
        # This calculates the values for the last layer of the transfer-learning model
...
if __name__ == '__main__':
    ...
    default_path = os.path.join(data_dir, 'cropped_panda.jpg')
    parser = argparse.ArgumentParser()
    parser.add_argument(
        '--image_path',
        type=str,
        default=default_path,
        help='Absolute path to jpg-image file.'
    )
    parser.add_argument(
        '--num_of_classes',
        type=int,
        default=5,
        help='How many classes to print.'
    )
    args = parser.parse_known_args()[0]
    begin = time()

    # Use the transfer-learning model to classify the image.
    pred = model.classify(image_path=args.image_path)

    # Print the scores and names for the top-5 predictions.
    model.print_scores(pred=pred, k=args.num_of_classes)

    # Print time it cost
    print('Time: ' + str(time() - begin) + ' s')
